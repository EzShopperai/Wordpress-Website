 ...
 */
class SasbyTheme_Woo_Shop_Settings extends SasbyTheme_Customizer {
    ...
    public function register_woo_shop_controls( $wp_customize ) {
        ...
        $wp_customize->add_control('products_cols_width',
			array(
			   'label'       => esc_html__('Products Per Column', 'sasby'),
			   'description' => esc_html__('Use product per col default 4', 'sasby'),
        ...
        $wp_customize->add_control('products_per_page',
			array(
			   'label'       => esc_html__('Number of items per page', 'sasby'),
			   'description' => esc_html__('Effect only for Shop custom page template ', 'sasby'),
			   'section'       => 'shop_layout_section',
			   'type'        => 'number'
			)
        );
...
if ( class_exists( 'WP_Customize_Control' ) ) {
	new SasbyTheme_Woo_Shop_Settings();
}
