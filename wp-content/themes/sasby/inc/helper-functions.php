...
if ( !class_exists( 'SasbyTheme_Helper' ) ) {
	
	class SasbyTheme_Helper {
		...
		public static function pagination( $wp_query = null) {
			...
			include SASBY_INC_DIR . 'views/pagination.php';
		}

		public static function comments_callback( $comment, $args, $depth ){
			include SASBY_INC_DIR . 'views/comments-callback.php';
		}
		...
		public static function fonts_url(){
			...
			$subsets = 'latin';
			$bodyFont = 'Manrope';
			$bodyFW = '500'; // Manrope:wght@300;400;500;600;700;800
			$menuFont = 'Manrope';
			$menuFontW = '600';
			$hFont = 'Manrope';
			$hFontW = '700';
			...
			// Body Font
			$body_font  = json_decode( SasbyTheme::$options['typo_body'], true );
			...
			// Menu Font
			$menu_font  = json_decode( SasbyTheme::$options['typo_menu'], true );
			...
			$h_font  = json_decode( SasbyTheme::$options['typo_heading'], true );
			...
			$h1_font  = json_decode( SasbyTheme::$options['typo_h1'], true );
			$h2_font  = json_decode( SasbyTheme::$options['typo_h2'], true );
			$h3_font  = json_decode( SasbyTheme::$options['typo_h3'], true );
			$h4_font  = json_decode( SasbyTheme::$options['typo_h4'], true );
			$h5_font  = json_decode( SasbyTheme::$options['typo_h5'], true );
			$h6_font  = json_decode( SasbyTheme::$options['typo_h6'], true );

			if ( 'off' !== _x( 'on', 'Google font: on or off', 'sasby' ) ) {
		...
		public static function socials(){
			$sasby_socials = array(
				'social_facebook' => array(
					'icon' => 'fa-facebook-f',
					'url'  => SasbyTheme::$options['social_facebook'],
				),
				'social_twitter' => array(
					'icon' => 'fa-x-twitter',
					'url'  => SasbyTheme::$options['social_twitter'],
				),
				'social_behance' => array(
					'icon' => 'fa-behance',
					'url'  => SasbyTheme::$options['social_behance'],
				),
				'social_dribbble' => array(
					'icon' => 'fa-dribbble',
					'url'  => SasbyTheme::$options['social_dribbble'],
				),
				'social_linkedin' => array(
					'icon' => 'fa-linkedin-in',
					'url'  => SasbyTheme::$options['social_linkedin'],
				),
				'social_youtube' => array(
					'icon' => 'fa-youtube',
					'url'  => SasbyTheme::$options['social_youtube'],
				),
				'social_pinterest' => array(
					'icon' => 'fa-pinterest-p',
					'url'  => SasbyTheme::$options['social_pinterest'],
				),
				'social_instagram' => array(
					'icon' => 'fa-instagram',
					'url'  => SasbyTheme::$options['social_instagram'],
				),
				'social_skype' => array(
					'icon' => 'fa-skype',
					'url'  => SasbyTheme::$options['social_skype'],
				),
				'social_rss' => array(
					'icon' => 'fas fa-rss',
					'url'  => SasbyTheme::$options['social_rss'],
				),
				'social_snapchat' => array(
					'icon' => 'fa-snapchat-ghost',
					'url'  => SasbyTheme::$options['social_snapchat'],
				),
				'social_tiktok' => array(
					'icon' => 'fa-tiktok',
					'url'  => SasbyTheme::$options['social_tiktok'],
				),
			);
			return array_filter( $sasby_socials, array( 'SasbyTheme_Helper' , 'filter_social' ) );
		}
		public static function team_socials(){
			$team_socials = array(
				'facebook' => array(
					'label' => esc_html__( 'Facebook', 'sasby' ),
					'type'  => 'text',
					'icon'  => 'fa-facebook-f',
				),
				'twitter' => array(
					'label' => esc_html__( 'Twitter', 'sasby' ),
					'type'  => 'text',
					'icon'  => 'fa-x-twitter',
				),
				'linkedin' => array(
					'label' => esc_html__( 'Linkedin', 'sasby' ),
					'type'  => 'text',
					'icon'  => 'fa-linkedin-in',
				),
				'behance' => array(
					'label' => esc_html__( 'Behance', 'sasby' ),
					'type'  => 'text',
					'icon'  => 'fa-behance',
				),
				'dribbble' => array(
					'label' => esc_html__( 'Dribbble', 'sasby' ),
					'type'  => 'text',
					'icon'  => 'fa-dribbble',
				),
				'skype' => array(
					'label' => esc_html__( 'Skype', 'sasby' ),
					'type'  => 'text',
					'icon'  => 'fa-skype',
				),
				'youtube' => array(
					'label' => esc_html__( 'Youtube', 'sasby' ),
					'type'  => 'text',
					'icon'  => 'fa-youtube',
				),
				'pinterest' => array(
					'label' => esc_html__( 'Pinterest', 'sasby' ),
					'type'  => 'text',
					'icon'  => 'fa-pinterest-p',
				),
				'instagram' => array(
					'label' => esc_html__( 'Instagram', 'sasby' ),
					'type'  => 'text',
					'icon'  => 'fa-instagram',
				),
				'github' => array(
					'label' => esc_html__( 'Github', 'sasby' ),
					'type'  => 'text',
					'icon'  => 'fa-github',
				),
				'stackoverflow' => array(
					'label' => esc_html__( 'Stackoverflow', 'sasby' ),
					'type'  => 'text',
					'icon'  => 'fa-stack-overflow',
				),
				'tiktok' => array(
					'label' => esc_html__( 'Tiktok', 'sasby' ),
					'type'  => 'text',
					'icon'  => 'fa-tiktok',
				),
			);
			return apply_filters( 'SasbyTheme_Helper', $team_socials );
		}

		public static function nav_menu_args(){
			$sasby_pagemenu = false;			
			if ( ( is_single() || is_page() ) ) {
				$layout_settings2 = get_post_meta( get_the_id(), 'sasby_layout_settings', true );
				if ( !empty( $layout_settings2 ) ) {
					$sasby_menuid = $layout_settings2['sasby_page_menu'];
				} else {
					$sasby_menuid = '';
				}
				if ( !empty( $sasby_menuid ) && $sasby_menuid != 'default' ) {
					$sasby_pagemenu = $sasby_menuid;
				}
			}
			if ( $sasby_pagemenu ) {
				$nav_menu_args = array( 'menu' => $sasby_pagemenu,'container' => 'nav' );
			}
			else {
		...
		public static function has_footer(){
			...
			if ( SasbyTheme::$footer_style == 2  ) {
				$footer_column = SasbyTheme::$options['footer_column_2'];
				...
				return false;
			} else if ( SasbyTheme::$footer_style == 3 ) {
				$footer_column = SasbyTheme::$options['footer_column_3'];
				for ( $i = 1; $i <= $footer_column; $i++ ) {
					if ( is_active_sidebar( 'footer-'. $i ) ) {
						return true;
					}
				...
				return false;				
			} else if ( SasbyTheme::$footer_style == 4 ) {
				$footer_column = SasbyTheme::$options['footer_column_4'];
				for ( $i = 1; $i <= $footer_column; $i++ ) {
					if ( is_active_sidebar( 'footer-'. $i ) ) {
						return true;
					}
				...
				return false;
			} else if ( SasbyTheme::$footer_style == 5 ) {
				$footer_column = SasbyTheme::$options['footer_column_5'];
				for ( $i = 1; $i <= $footer_column; $i++ ) {
					if ( is_active_sidebar( 'footer-'. $i ) ) {
						return true;
					}
		...
		public static function get_font_css( $filename ){
			$path = '/assets/fonts/flaticon-sasby/' . $filename . '.css';
		...
		public static function has_active_widget(){
			...
			if ( ( class_exists( 'WooCommerce' ) && is_shop() ) || ( class_exists( 'WooCommerce' ) && is_product_category() ) || ( class_exists( 'WooCommerce' ) && is_product_tag() ) || ( class_exists( 'WooCommerce' ) && is_product() ) ) {	
				...
				if ( SasbyTheme::$sidebar ) {
					if ( is_active_sidebar( SasbyTheme::$sidebar ) ) {
						$sasby_layout_class = 'col-xl-8';
					} else {
						$sasby_layout_class = 'col-sm-12 col-12';
					}
	...
	public static function shop_list_page_url()
		...
		public static function custom_sidebar_fields(){
			...
			$sidebar_fields['sidebar'] = esc_html__( 'Sidebar', 'sasby' );

			$sidebars = get_option( 'sasby_custom_sidebars', array() );
			if ( $sidebars ) {
		...
		public static function dynamic_internal_style(){
			ob_start();
			include SASBY_INC_DIR . 'variable-style.php';
			$dynamic_css  = ob_get_clean();
			$dynamic_css  = self::minified_css( $dynamic_css );
			wp_register_style( 'sasby-dynamic', false );
			wp_enqueue_style( 'sasby-dynamic' );
			wp_add_inline_style( 'sasby-dynamic', $dynamic_css );			
		}
