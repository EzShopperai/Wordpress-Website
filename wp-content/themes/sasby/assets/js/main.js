    ...
    if (typeof $.fn.magnificPopup == 'function') {
        if ($('.zoom-gallery').length) {
            $('.zoom-gallery').each(function () { // the containers for all your galleries
                $(this).magnificPopup({
                    delegate: 'a.sasby-popup-zoom', // the selector for gallery item
                    type: 'image',
                    gallery: {
                        enabled: true
                    }
    ...
    if( $('.header-shop-cart').length ){
        $( document ).on('click', '.remove-cart-item', function(){
          ...
          $.ajax({
            type: 'post',
            dataType: 'json',
            url: sasbyObj.ajaxURL,
            data: { action: "sasby_product_remove", 
                product_id: product_id
            },success: function(data){
              main_parent.html( data["mini_cart"] );
              $( document.body ).trigger( 'wc_fragment_refresh' );
            },error: function(xhr, status, error) {
              $('.header-shop-cart').children('ul.minicart').html('<li class="cart-item"><p class="cart-update-pbm text-center">'+ sasbyObj.cart_update_pbm +'</p></li>');
            }
          });
          return false;
        }); 
...
function sasby_load_content_area_scripts($) {
    /* progress circle */
...
//function Load
function sasby_content_load_scripts() {
    var $ = jQuery;

    // Preloader
    $("#preloader").css("height", "0").fadeOut("slow", function () {
        $(this).remove();
    });
   ...
   ======================================*/
    ...
    $('.sasby-horizontal-slider').each(function(){
...
(function ($) {
    ...
    if ($(".ajax-scroll-post").length > 0) {
        ...
        $(window).scroll(function () {
            ...
            if (current_post >= sasbyObj.post_scroll_limit) {
                onScrollPagi = false;
                return;
            }

            if ($(document).scrollTop() > ($(document).height() - bottomOffset) && onScrollPagi == true) {
                let cat_ids = $('input#sasby-cat-ids').val();
                $.ajax({
                    url: sasbyObj.ajaxURL,
                    data: {
                        action: 'sasby_single_scroll_post',
